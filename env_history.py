            ...
            if item['label'] == 'state':
                ...
                if use_tech:
                    ...
                    for k, v in tech_dict.items():
                        state_log += f', {k}: {v}'
                price_s += state_log + '\n'
        price_s += f'Provide a brief report to analyze the recent information and estimate the market trend accordingly.'

        state = self._history[-1]['value']
        news_s = f"You are an ETH cryptocurrency trading analyst. You are required to analyze the following news articles: {state['news']}. Provide a brief report to analyze the news and estimate the market trend accordingly."
